...
public class SearchTestIncorrectAttributeSelected extends BasicTestCase {
  ...
  public void test() throws Exception {
    ...
    testResultNotFound(searchQuery);
  }
  ...
  private void setMetaClassesAndAttributes(CapellaSearchSettings searchSettings) {
    Set<Object> searchMetaClasses = SearchForItemCache.getInstance().getClassItems().stream()
        .filter(item -> CtxPackage.Literals.SYSTEM_FUNCTION.isSuperTypeOf((EClass) item.getObject()))
  ...
  private void testResultNotFound(CapellaSearchQuery searchQuery) {
